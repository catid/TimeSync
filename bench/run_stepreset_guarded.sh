...
OUT_DIR="${1:-${ROOT_DIR}/benchmarks/run_stepreset_guarded_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "${OUT_DIR}"
...
"${PYTHON}" "${ROOT_DIR}/bench/report_steps_compare.py" "${MERGED}" \
  --require-poll --methods "M4_TimeSync,M4_TimeSyncStepReset,M4_TimeSyncStepReset:guard_xor,M4_TimeSyncStepReset:sticky" \
  --out "${OUT_DIR}/steps_compare.md"

printf "\nGuarded StepReset sweep outputs: %s\n" "${OUT_DIR}"
