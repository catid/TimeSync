...
OUT_DIR="${1:-${ROOT_DIR}/benchmarks/run_shadow_cse_step_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "${OUT_DIR}"
...
METHODS=(
  "M4_TimeSync"
  "M4_TimeSyncShadowSkew"
  "M4_TimeSyncShadowSkew:cse"
  "M4_TimeSyncShadowSkew:cse_step"
)
...
"${PYTHON}" "${ROOT_DIR}/bench/report_steps_compare.py" "${MERGED}" \
  --require-poll --methods "M4_TimeSync,M4_TimeSyncShadowSkew,M4_TimeSyncShadowSkew:cse,M4_TimeSyncShadowSkew:cse_step" \
  --out "${OUT_DIR}/steps_compare.md"

printf "\nShadow+CSE+Step sweep outputs: %s\n" "${OUT_DIR}"
