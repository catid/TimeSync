...
OUT_DIR="${1:-${ROOT_DIR}/benchmarks/run_clock_step_mc_side_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "${OUT_DIR}"
...
GRID="${GRID:-0}"
EXACT="${EXACT:-1}"
SCENARIO="E30_clock_step_mc_side"
...
"${PYTHON}" "${ROOT_DIR}/bench/report_clock_step_mc.py" "${MERGED}" \
  --scenario "${SCENARIO}" --out "${OUT_DIR}/clock_step_mc_side.md"

printf "\nClock-step MC (random side) outputs: %s\n" "${OUT_DIR}"
