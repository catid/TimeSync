...
OUT_DIR="${1:-${ROOT_DIR}/benchmarks/run_clock_step_mc_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "${OUT_DIR}"

SEEDS="${SEEDS:-20}"
THREADS="${THREADS:-$(nproc)}"
GRID="${GRID:-0}"
SCENARIO="${SCENARIO_OVERRIDE:-E26_clock_step_mc}"
...
"${PYTHON}" "${ROOT_DIR}/bench/report_clock_step_mc.py" "${MERGED}" \
  --scenario "${SCENARIO}" --out "${OUT_DIR}/clock_step_mc.md"

printf "\nClock-step MC outputs: %s\n" "${OUT_DIR}"
