...
OUT_DIR="${1:-${ROOT_DIR}/benchmarks/run_stepreset_holdoff_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "${OUT_DIR}"
...
METHODS=(
  ...
  "M4_TimeSyncStepReset:hold0p1s"
  "M4_TimeSyncStepReset:hold1s"
  "M4_TimeSyncStepReset:hold5s"
  "M4_TimeSyncStepReset:hold10s"
  "M4_TimeSyncStepReset:guard_xor"
)
...
"${PYTHON}" "${ROOT_DIR}/bench/report_steps_compare.py" "${MERGED}" \
  --require-poll --methods "M4_TimeSync,M4_TimeSyncStepReset,M4_TimeSyncStepReset:hold0p1s,M4_TimeSyncStepReset:hold1s,M4_TimeSyncStepReset:hold5s,M4_TimeSyncStepReset:hold10s,M4_TimeSyncStepReset:guard_xor" \
  --out "${OUT_DIR}/steps_compare.md"

printf "\nStepReset holdoff sweep outputs: %s\n" "${OUT_DIR}"
