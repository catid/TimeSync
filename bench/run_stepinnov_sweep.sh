...
OUT_DIR="${1:-${ROOT_DIR}/benchmarks/run_stepinnov_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "${OUT_DIR}"
...
"${PYTHON}" "${ROOT_DIR}/bench/report_steps_compare.py" "${MERGED}" \
  --require-poll --methods "M4_TimeSync,M4_TimeSyncStepGuard,M4_TimeSyncStepGuard:mad8" \
  --out "${OUT_DIR}/steps_compare.md"

printf "\nStep innovation sweep outputs: %s\n" "${OUT_DIR}"
